// THE SPRITES' HOTNESS: each sprite measured in the page against the house number (OFF = the live glyph at brightness(2), no halo; ON = the sprite), then the bake scale per glyph.
...
(async () => {
  ...
  for (const [id, host, glyphSel, prep] of GL) {
    ...
    await set(`${host} ${glyphSel} { opacity: 1 !important; filter: brightness(2) !important; } ${host} img.bs-lit { opacity: 0 !important; }`); const off = decodePNG(await h.p.screenshot(clip, 2));
    await set(`${host} ${glyphSel} { opacity: 0 !important; } ${host} img.bs-lit { opacity: 1 !important; }`); const on = decodePNG(await h.p.screenshot(clip, 2));
